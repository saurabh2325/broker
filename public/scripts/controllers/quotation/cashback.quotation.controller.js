(function(){
    'use strict';
    angular.module('app')
    .controller('cashbackQuoteCtrl', ["$q", "$scope", "$log", "$timeout", "cashbackQuotation", function ($q, $scope, $log, $timeout, cashbackQuotation) {
        $scope.templateUrl = './views/pages/quotation/cashback/cashbackAdd.html?v='+window.app_version
        $scope.subtitle = "List"
        ...
        $scope.nextPanel= function(valid){
            if(valid){
                ...
                $scope.templateUrl = './views/pages/quotation/cashback/cashbackAdd1.html?v='+window.app_version
            }else{
        ...
        function saveQuote(valid, quoteForm){
            ...
            $scope.errorMsg = false;
            if(valid){
                cashbackQuotation.create($scope.quoteData).then(function(response){
