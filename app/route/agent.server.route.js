...
// Define storage path of upload agent image
...
// Upload agent image function
var upload = multer({ storage: imageStorage });
route.post('/uploadAgentImg', upload.single('file'), function(request, response){
...
// Agent Registration  
route.post('/insertAgent', function(request, response){
    var agentModel = new agent();
    agentModel.uid = request.body.uid;
    agentModel.company_name = request.body.company_name;
    agentModel.company_contact_no = request.body.company_contact_no;
	agentModel.first_name = request.body.first_name;
    agentModel.last_name = request.body.last_name;
    agentModel.email = request.body.email;
    agentModel.territory = request.body.territory;  
    agentModel.permission = request.body.permission;
    agentModel.bank_detail = request.body.bank_detail;
    agentModel.description = request.body.description;
    agentModel.work_contact_no = request.body.work_contact_no;
    agentModel.mobile_contact_no = request.body.mobile_contact_no;  
    agentModel.address = request.body.address;
    agentModel.status = request.body.status;
    agentModel.image = request.body.image;
    agentModel.create_date = Date.now();
    agentModel.temporary_token = jwt.sign({ uid: request.body.uid, first_name: request.body.first_name, last_name: request.body.last_name, email: request.body.email }, secrets, { expiresIn: '24h' }); // Create a token for activating account through e-mail
    if(agentModel.uid && agentModel.first_name && agentModel.last_name && agentModel.email){
        agentModel.save(function(err, agent){
            if(err){
                ...
                var email = {
                    ...
                    text: 'Hello ' + agent.first_name + ' ' + agent.last_name + ', Thank you.',
                    html: `<!DOCTYPE html>
                            <html>
                                <head>
                                ...
                                <body>
                                    <table class="table">
                                        <tbody>
                                            <tr>
                                                <td>
                                                    <h3 style="color: darkslategray;margin: 10px 0px 5px;"> Dear` + agent.first_name + ' ' + agent.last_name + `</h3>
                                                </td>
                                            </tr>
                                            <tr>
                                                <td>
                                            ...
                                            <tr>
                                                <td style="padding-top: 20px;">
                                                    Please confirm your email address. <a href="http://localhost:8030/agent/activate/` + agent.temporary_token + `"><button class="btn btn-confirm">Confirm Account</button> </a>
                                                </td>
                                            </tr>
                                            <tr>
                                                <td style="padding-top: 20px;">
                ...
                client.sendMail(email, function(err, info) {
                    if (err) {
                        ...
                        console.log('sent to: ' + agent.email); // Log e-mail 
                    }
                });
                response.json({ success: true, message:'New agent successfully created'});
            }
        }) 
    }else{
...
route.put('/agent/activate/:token', function(request, response){
    agent.findOne({temporary_token: request.params.token}, function(err, user){
        if(err) throw err;
...
route.get('/agentList', function(request, response){
    agent.find({}, function(err, agentList){
        if(err){
            response.json({success: false, message: 'Something went wrong !, please contact to your service agentistrator'});
        }else{
            var b = agentList.length;
            if(!b){
                response.json({success: true, message: 'Sorry!, No artist available at this time.'});
            }else{
                response.json({success: true, data: agentList});
            }      
        }
    })
});

// function for getting agent Detail 
route.get('/agentGetById/:id', function(request, response){
    var id = request.params.id;
    agent.findOne({_id: id}, function(err, agentDetail){
        if(err){
            response.json({success: false, message: 'Something went wrong !, please contact to your service agentistrator'});
        }
        else{
            response.json({success: true, data: agentDetail}); 
        }
    });
});

// function for updateing Detail
route.put('/updateAgent', function(request, response){
    var id = request.body._id;
    agent.findById(id, function(err, agent){
        if(err){
          response.json({success: false, message: 'Agent detail not found'});
        }
        else{
            agent.update(request.body ,function(err, success){
                if(err){
                    response.json({success: false, message: err});
                }
                else{
                    response.json({success: true, data:'Agent detail update successfully'});
                }
            });
        }
    })
...
route.delete('/deleteAgent/:id', function(request, response){
    var id = request.params.id;
    agent.remove({_id: id}, function(err, success){
        if(err){
            response.json({success: false, message: err});
        }
        else{
...
route.put('/sendAgentResetLink', function(request, response) {
    agent.findOne({ email: request.body.email }).select('uid email reset_token first_name last_name').exec(function(err, user) {
        if (err) {
...
// function to send reset link
route.put('/resetAgentPassword/:resettoken', function(request, response) {
    var id = request.params.resettoken;
    agent.findOne({ reset_token: id }).select('uid first_name last_name email reset_token').exec(function(err, user) {
        if (err) {
            // Function to send e-mail to myself
            response.json({ success: false, message: 'Something went wrong. This error has been logged and will be addressed by our staff. We apologize for this inconvenience!' });
        }else{
