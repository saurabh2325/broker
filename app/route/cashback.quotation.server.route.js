...
var quotation = require('../../app/model/cashback.quotation.server.model');
var route = express.Router();

// Quotation Post Method
route.post('/insertCashbackQuote', function(req, response) {
    ...
    if (quotationModel.quotation_id) {
        quotationModel.save(function(err, quotation) {
            if (err) {
                ...
                response.json({ success: true, message: 'Congratulations!, Cashback quotation created successfully.' });
            }
        })
    } else {
        response.json({ success: false, message: 'Sorry!, Cashback quotation not created. Please  ' });
    }
})

// This Function is Used for get the Single Quotation  Value and depend on Id 
route.get('/cashbackQuoteGetById/:id', function(request, response) {
...
route.get('/cashbackQuoteList', function(request, response) {
...
route.put('/updateCashbackQuote', function(request, response) {
...
route.delete('/deleteCashbackQuote/:id', function(request, response) {
    var id = request.params.id;
