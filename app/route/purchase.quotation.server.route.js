...
var quotation = require('../../app/model/purchase.quotation.server.model');
var route = express.Router();

// Quotation Post Method
route.post('/insertPurQuote', function(req, response) {
    ...
    if (quotationModel.quotation_id) {
        quotationModel.save(function(err, quotation) {
            if (err) {
                ...
                response.json({ success: true, message: 'Congratulations!, Purchase quotation created successfully.' });
            }
        })
    } else {
        response.json({ success: false, message: 'Sorry!, Purchase quotation not created. Please  ' });
    }
})

// This Function is Used for get the Single Quotation  Value and depend on Id 
route.get('/purQuoteGetById/:id', function(request, response) {
...
route.get('/purQuoteList', function(request, response) {
...
route.put('/updatePurQuote', function(request, response) {
...
route.delete('/deletePurQuote/:id', function(request, response) {
    var id = request.params.id;
